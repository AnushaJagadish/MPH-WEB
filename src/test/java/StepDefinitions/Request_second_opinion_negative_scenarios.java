...
public class Request_second_opinion_negative_scenarios extends Generic_function{
	...
	/* Validate that the user is able to Login with valid data */
	@When("user enters the valid phonenumber and password")
	public static void user_enters_valid_credentials() throws InterruptedException, Exception {
		try {
			...
			e.printStackTrace();
			takeScreenShot("request_second_opinion_negative_user_enters_valid_credentials");
		}
	}
	@Then("clicks on 'login' button")
	public static void clicks_on_login() throws InterruptedException, Exception {
		try {
			click("login");
			browser_wait(1000);
		} catch (Exception e) {
			e.printStackTrace();
			takeScreenShot("request_second_opinion_negative_clicks_on_login");
		}
	}
